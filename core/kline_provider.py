...
try:
    ...
    class KLType:
        ...
        K_30M = "K_30M"; K_60M = "K_60M"
        K_DAY = "K_DAY"; K_WEEK = "K_WEEK"
    class AuType:
        QFQ = "QFQ"; HFQ = "HFQ"; NONE = "NONE"
...
class KlineProvider:
    ...
    def get_daily(self, vt_symbol: str, days: int = DEFAULT_DAILY_DAYS) -> pd.DataFrame:
    ...
    def _fetch_from_futu(self, vt_symbol: str, ktype: str, days: int) -> pd.DataFrame:
        if self.ctx is None:
            return pd.DataFrame()
        futu_code = self.vt_to_futu(vt_symbol)
        now = time.time()
    ...
    def stats(self) -> dict:
        ...
        return {**self._stats, "total_requests": total,
