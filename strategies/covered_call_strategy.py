"""
strategies/covered_call_strategy.py - v2.6.0
备兑开仓策略：持有正股时卖出虚值Call，收取权利金增强收益
"""
import time
...
class CoveredCallStrategy:
    ...
    def __init__(self, strategy_engine, strategy_name, vt_symbol, setting):
        ...
        self.call_strike_percent = setting.get("call_strike_percent", 1.08)
        self.expiry_days = setting.get("expiry_days", 30)
        self.premium_target = setting.get("premium_target", 0.02)
        self.fixed_size = setting.get("fixed_size", 100)
        self.max_position = setting.get("max_position", 10)
        self.min_cash_reserve = setting.get("min_cash_reserve", 50000)
    ...
    def on_init(self):
        ...
        self.write_log(f"  正股: {self.vt_symbol}")
        self.write_log(f"  Call行权价比例: {self.call_strike_percent}")
    ...
    def on_tick(self, tick: TickData):
        ...
        if not self._is_trading_time():
            return
        if self.stock_pos < self.fixed_size:
            self.write_log(f"正股不足: 需要 {self.fixed_size}, 持有 {self.stock_pos}")
            return
    ...
    def _try_sell_call(self):
        if self._underlying_price <= 0:
            return

        target_strike = round(self._underlying_price * self.call_strike_percent, 1)
        option_symbol = self._find_call_option(target_strike)

        if not option_symbol:
            self.write_log(f"未找到合适的Call合约 (行权价≈{target_strike})")
            return

        self.write_log(
            f"卖出Call: {option_symbol} | "
            f"行权价 {target_strike} | "
            f"数量 {self.fixed_size}股 | "
    ...
    def _find_call_option(self, target_strike: float) -> str:
        try:
            contracts = self.main_engine.get_all_contracts()
            best_symbol = ""
            best_diff = float("inf")
            for contract in contracts:
                if getattr(contract, 'option_type', '') != "CALL":
                    continue
