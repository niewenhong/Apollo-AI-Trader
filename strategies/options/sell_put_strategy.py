# -*- coding: utf-8 -*-
"""
strategies/options/sell_put_strategy.py - Apollo-AI-Trader v3.1.1
...
- 注意：此文件类名是 SellPutStrategy（不是 SellCallStrategy！）
"""
...
class SellPutStrategy(BaseOptionStrategy):
    ...
    def on_init(self):
        """策略初始化：加载历史K线"""
        self.write_log("[SellPut] on_init 开始，加载历史K线")
        try:
            ...
            self.write_log("[SellPut] load_bar 异常: %s" % e)
        self._init_retry_count = 0
        self.write_log("[SellPut] on_init 完成")
    ...
    def _on_5m_bar_impl(self, bar: BarData):
        ...
        if adx > self.adx_trend_threshold:
            self.write_log("[SellPut] ADX=%.1f 强趋势，暂停卖Put" % adx)
        ...
        if self.legs and not self._rolling:
            for name, leg in list(self.legs.items()):
                ...
                if delta > self.roll_when_ditm:
                    self.write_log("[SellPut] %s delta=%.2f>%.2f，展期" %
    ...
    def _enough_cash(self, bar: BarData) -> bool:
        ...
        ok = avail >= need
        if not ok:
            self.write_log("[SellPut] 现金不足 需要≈%.0f 可用=%.0f" % (need, avail))
        return ok

    # ── 选合约 ────────────────────────────────────
    def _find_put_to_sell(self, bar: BarData):
        if self._init_retry_count >= 5:
            self.write_log("[SellPut] 初始化重试已达上限，跳过本次开仓")
            return
        ...
        if chain is None:
            self.write_log("[SellPut] %s 期权链为空" % code)
            return
        ...
        if not puts:
            self.write_log("[SellPut] 无符合条件Put")
            return
        ...
        if not in_band:
            self.write_log("[SellPut] 无满足严格delta区间的Put（保守跳过）")
            return
        ...
        if annual_roi < self.min_annual_roi:
            self.write_log("[SellPut] ROI=%.1f%%<%.0f%%，跳过" %
    ...
    def _check_tick_exit(self, bar: BarData = None):
        ...
        if cost <= 0:
            return
        if cur_pnl < -cost * 0.8:
            self.write_log("[SellPut] 止损 cur=%.0f cost=%.0f" % (cur_pnl, cost))
            self._close_all_legs()
            self._rolling = False
        elif cur_pnl > self.net_premium * 0.7:
            self.write_log("[SellPut] 止盈 cur=%.0f prem=%.0f" % (cur_pnl, self.net_premium))
            self._close_all_legs()
            self._rolling = False
